...
from backend.app.tools import (
    FRIENDLY_LABELS, HallucinationError, PS_RE, openai_tool_schemas, run_tool,
...
async def chat_stream(session_id: str, user_message: str) -> AsyncIterator[dict]:
    ...
    client = get_client()
    messages = [{"role": "system", "content": prompts.SYSTEM_PROMPT}] + state["messages"][-MAX_HISTORY_MESSAGES:]
    final_text = ""
    ...
    for _round in range(MAX_TOOL_ROUNDS + 1):
        ...
        for call in tool_calls:
            yield {"event": "tool_start", "data": {"name": call["name"],
                   "label": FRIENDLY_LABELS.get(call["name"], "Working…")}}
        ...
        for call, result in zip(tool_calls, results):
            ...
            messages.append({"role": "tool", "tool_call_id": call["id"],
                             "name": call["name"], "content": _compact(result)})
    ...
    try:
        ...
        messages.append({"role": "system",
                         "content": prompts.HALLUCINATION_NUDGE.format(numbers=", ".join(err.numbers))})
        retry: list[str] = []
        async for kind, payload in client.chat(messages, openai_tool_schemas()):
            if kind == "token":
...
def _log_hallucination(session_id: str, user: str, draft: str, numbers: set[str]) -> None:
    ...
    with HALLUCINATION_LOG.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps({"ts": time.time(), "session": session_id, "user": user,
                             "numbers": sorted(numbers), "draft": draft[:500]}) + "\n")
