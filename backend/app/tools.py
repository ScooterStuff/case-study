...
class PartDetailsIn(BaseModel):
    part_identifier: str = Field(description="PS number, manufacturer part number, or an old superseded number")
...
def _card(p: dict) -> dict:
    return {k: p.get(k) for k in (
        "ps_number", "mpn", "brand", "title", "price", "availability",
        "install_difficulty", "install_time", "rating", "review_count",
        "image_url", "product_url", "appliance_type")}
...
def search_parts(query: str, appliance_type: str | None = None) -> dict:
    ...
    return {"data": {"results": [_card(p) for p in parts], "query": query},
...
def get_part_details(part_identifier: str) -> dict:
    ...
    if part is None:
        return {"data": {"found": False, "identifier": part_identifier,
                         "note": "No part with that number in our catalog. Ask the user to double-check it."},
                "ui_block": None}
    return {"data": {"found": True, **part},
            "ui_block": {"type": "product_card", "product": _card(part),
...
def check_compatibility(part_identifier: str, model_number: str) -> dict:
    ...
    return {"data": {"status": res.status, "ps_number": res.ps_number,
                     "model_number": res.model_number, "evidence": res.evidence,
                     "honesty_note": honesty, "parts_verified_for_model_sample": sample},
            "ui_block": {"type": "compat_result", "verdict": res.status,
                         "part": res.ps_number, "model": res.model_number,
...
def diagnose_issue(symptom_description: str, appliance_type: str, brand: str | None = None) -> dict:
    ...
    if chunks:
        ...
        causes = [{"symptom": c["symptom"], "rank": c["rank"], "cause": c["cause"],
                   "detail": (c["body"] or "")[:600]} for c in full][:6]
        if not causes:  # only summary-level guides matched
            causes = [{"symptom": c["symptom"], "rank": i + 1, "cause": c["symptom"],
                       "detail": c["document"][:600]} for i, c in enumerate(chunks[:4])]
    ...
    return {"data": {"causes": causes, "suggested_parts": [_card(p) for p in parts]},
            "ui_block": {"type": "diagnosis",
...
def get_installation_guide(part_identifier: str) -> dict:
    ...
    data = {"found": True, "ps_number": part["ps_number"], "title": part["title"],
            "difficulty": part.get("install_difficulty"), "time": part.get("install_time"),
            "video_url": part.get("video_url"), "product_url": part.get("product_url"),
            "customer_stories": stories}
    return {"data": data,
            "ui_block": {"type": "install_guide", "part": _card(part),
                         "difficulty": data["difficulty"], "time": data["time"],
                         "video_url": data["video_url"], "stories": stories}}
...
def order_support(action: str, order_id: str | None = None, email: str | None = None) -> dict:
    ...
    return {"data": data,
            "ui_block": {"type": "order_status", **data}}


# ----------------------------------------------------------- registry/schema

TOOLS = {
    "search_parts": (search_parts, SearchPartsIn, "Search the parts catalog with natural language (hybrid keyword+semantic)."),
    "get_part_details": (get_part_details, PartDetailsIn, "Exact lookup of one part by PS number / manufacturer number / superseded number."),
    "check_compatibility": (check_compatibility, CompatibilityIn, "Deterministic check whether a part fits an appliance model (database join, never a guess)."),
    "diagnose_issue": (diagnose_issue, DiagnoseIn, "Map a symptom description to likely causes (ranked) and candidate parts."),
    "get_installation_guide": (get_installation_guide, InstallGuideIn, "Installation difficulty, time, video and real customer repair stories for a part."),
    "order_support": (order_support, OrderSupportIn, "Order status / returns / cancellation (requires order_id)."),
}
