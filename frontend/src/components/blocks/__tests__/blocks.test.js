...
test("ProductCard shows price, stock badge and difficulty chip", () => {
  ...
  expect(
    screen.getByRole("img", { name: /Lower Dishrack Wheel/ }),
  ).toBeInTheDocument();
});
...
test("CompatResult no_match_found shows the honesty note, never a hard no", () => {
  ...
  expect(screen.getByText("⚠")).toBeInTheDocument();
  expect(
    screen.getByText(/doesn't necessarily mean it won't fit/),
  ).toBeInTheDocument();
});
...
test("Diagnosis renders causes in rank order with suggested parts", () => {
  wrap(
    <Diagnosis
      block={{
        causes: [
          { rank: 1, cause: "Water Fill Tubes" },
          { rank: 2, cause: "Water Inlet Valve" },
        ],
